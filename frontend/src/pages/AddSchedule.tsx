...
const initialCustomers: Customer[] = [
  { id: "C-001", name: "Text Customer 2", phone: "966000000002", email: "mt@brikon.net", balance: 0.00, status: "Paid Jobs" },
  { id: "C-002", name: "Osool Real Estate", phone: "966500000001", email: "osool@example.com", balance: 1250.00, status: "Billing Contract" },
  { id: "C-003", name: "Al Rajhi Commercial", phone: "966550000003", email: "rajhi@example.com", balance: -320.00, status: "Paid Jobs" }
]
...
export default function AddSchedule() {
  ...
  const [selectedCustomerId, setSelectedCustomerId] = useState<string>("C-001")
  ...
  const basePrice = useMemo(() => {
    switch (selectedService) {
      case "HVAC Service": return 250
      case "Deep Cleaning Service": return 350
      case "Pest Control Service": return 200
      case "Electrical Repair": return 180
      default: return 150
    }
  }, [selectedService])
  ...
  const handleDone = () => {
    ...
    const payload = {
      ...
      notes,
      comment,
      contract: hasContractEndDate ? `Yes (Ends ${contractEndDate})` : "No",
    }

    console.log("Saving schedule payload:", payload)
    toast.success("Schedule Created successfully!", {
      description: `Job: ${payload.jobTitle} is scheduled on ${payload.activeDate} for ${payload.customerName}.`
    })

    // Navigate back to calendars or jobs
    setTimeout(() => {
    ...
    <div className="space-y-6 pb-20">
      ...
      <div className="flex flex-col xl:flex-row gap-4 justify-between bg-white dark:bg-gray-950 p-4 rounded-xl border border-gray-200 dark:border-gray-800 shadow-xs">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg px-3 py-2">
            <User className="h-4 w-4 text-gray-500" />
            <select
              value={selectedCustomerId}
              onChange={(e) => setSelectedCustomerId(e.target.value)}
              className="bg-transparent text-sm font-semibold focus:outline-hidden"
            >
              {customers.map((c) => (
          ...
          <Button 
            variant="link" 
            className="text-[#ef801f] hover:text-[#c2410c] text-xs font-semibold p-0 h-auto"
            onClick={() => toast.info("Client editing profile modal is opened.")}
          >
            Edit Customer
          </Button>

          <span className="text-[11px] text-gray-400 block w-full sm:w-auto">
            search items must contain at least 3 characters
          </span>
        </div>
        ...
        <div className="flex items-center justify-center gap-2">
          ...
          <div className="flex items-center gap-2 cursor-pointer font-medium text-sm text-gray-700 dark:text-gray-300">
        ...
        <div className="flex flex-wrap items-center gap-3 justify-end xl:w-auto w-full">
          <div className="text-right mr-2 hidden md:block">
            <p className="text-xs text-[#ef801f] font-semibold">{activeCustomer.name}</p>
            <p className="text-[11px] text-gray-400">{activeCustomer.email}</p>
          </div>
          
          <Badge className="bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 rounded-md font-semibold">
            SAR {activeCustomer.balance.toFixed(2)}
          </Badge>
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <div className="h-8 w-8 rounded-full bg-[#001e60] text-white flex items-center justify-center font-bold text-xs uppercase cursor-pointer">
              {activeCustomer.name.slice(0, 2)}
            </div>
            <Button className="bg-[#ef801f] hover:bg-[#c2410c] text-white h-8 text-xs font-semibold px-4" onClick={handleDone}>
              Add
            </Button>
          </div>
        </div>
      ...
      {/* --- SINGLE COLUMN FORM WORKSPACE --- */}
      <div className="bg-white dark:bg-gray-950 p-6 rounded-xl border border-gray-200 dark:border-gray-800 shadow-2xs space-y-6 text-left">
        ...
        <div className="space-y-4 pt-4 border-t border-gray-100 dark:border-gray-900">
          ...
          <div className="space-y-3">
            <div className="space-y-1.5">
              ...
              <Select value={selectedService} onValueChange={setSelectedService}>
                ...
                <SelectContent>
                  <SelectItem value="HVAC Service" className="text-xs">HVAC Service</SelectItem>
                  <SelectItem value="Deep Cleaning Service" className="text-xs">Deep Cleaning Service</SelectItem>
                  <SelectItem value="Pest Control Service" className="text-xs">Pest Control Service</SelectItem>
                  <SelectItem value="Electrical Repair" className="text-xs">Electrical Repair</SelectItem>
                </SelectContent>
              </Select>
            </div>
          ...
          <div className="space-y-3">
            ...
            <div className="bg-gray-50 dark:bg-gray-900/50 p-3 rounded-lg space-y-2 text-xs">
              ...
              {appliedDiscount > 0 && (
                <div className="flex justify-between items-center text-red-500">
                  ...
                  <span>- SAR {appliedDiscount.toFixed(2)}</span>
                </div>
              )}
              <div className="flex justify-between items-center pt-1.5 border-t border-dashed border-gray-200 dark:border-gray-800">
                ...
                <span>SAR {untaxedAmount.toFixed(2)}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-gray-400">VAT ({taxRate}%):</span>
                <span>SAR {taxAmount.toFixed(2)}</span>
              </div>
              <div className="flex justify-between items-center pt-2 border-t font-bold text-gray-800 dark:text-gray-200">
                <span>Total Cost:</span>
                <span className="text-[#ef801f]">SAR {totalAmount.toFixed(2)}</span>
              </div>
            </div>
          </div>
            ...
            {selectedCrewIds.length > 0 && (
              <div className="flex flex-wrap gap-1 mt-2">
                {selectedCrewIds.map((cid) => {
                  const cr = mockCrews.find(c => c.id === cid)
                  return (
                    <Badge 
                      key={cid} 
